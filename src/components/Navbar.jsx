...
const Navbar = () => {
    ...
    const handleSwitchToUser = () => {
     ...
     const renderDesktopMenu = () => {
          if (!user) {
               ...
               return (
                    <>
                         ...
                         <Link to="/register" className="border border-gray-400 px-4 py-1 rounded-full text-sm hover:bg-gray-100">
                         Join us
                         </Link>
                    </>
               );
          ...
          return (
               <>
                    <Link to="/freelancer/register" className="border border-gray-400 px-4 py-1 rounded-full text-sm font-semibold hover:bg-gray-100 mr-4">
                         Start to Freelance
                    </Link>
               ...
               <div className="relative group">
                    <button className="flex items-center space-x-2 focus:outline-none">
                         ...
                         {user.avatar ? (
                              ...
                              <span className="text-lg text-gray-700">{user.name ? user.name[0].toUpperCase() : "U"}</span>
                         )}
                         </span>
                    </button>
                    <div className="absolute right-0 mt-2 w-48 bg-white border rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity z-10">
                         {role === 'freelancer' ? (
                         <>
                              <Link to="/freelancer/dashboard" className="block px-4 py-2 text-gray-800 hover:bg-gray-100">Dashboard</Link>
                              <Link to="/freelancer/profile" className="block px-4 py-2 text-gray-800 hover:bg-gray-100">Freelancer Profile</Link>
                              <Link to="/freelancer/earnings" className="block px-4 py-2 text-gray-800 hover:bg-gray-100">Pendapatan</Link>
                         </>
                         ) : (
                         <>
                              <Link to="/profile" className="block px-4 py-2 text-gray-800 hover:bg-gray-100">Profile</Link>
                              <Link to="/transactions" className="block px-4 py-2 text-gray-800 hover:bg-gray-100">Riwayat Transaksi</Link>
                         </>
                         )}
     ...
     return (
          <header className="border-b border-gray-300">
               ...
               <nav className="max-w-screen-xl mx-auto px-6 py-4 flex items-center justify-between">
                    <img src={creatifyLogo} alt="Creatify Logo" className="w-25" />
